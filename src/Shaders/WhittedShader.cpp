...
RGB WhittedShader::DoExecute(const Ray& ray, const Scene& scene, const Intersection& intersection, int depth) const
{
  RGB color = RGB{0.0f};
  if (depth > MAX_DEPTH)
  {
...
RGB WhittedShader::DirectIllumination(const Ray& ray, const Scene& scene, const Intersection& intersection, const Material& material) const
{
  ...
  for (const auto& light : scene.GetLights())
  {
    ...
    else if (light_type == LightType::Point)
    {
      ...
      const Vector direction = glm::normalize(light_position - intersection.Position);
      const float light_distance = glm::length(light_position - intersection.Position);

      if (const float cos_light = glm::dot(direction, shading_normal); cos_light > 0)
      {
        const auto shadow_ray = Ray::WithOffset(intersection.Position, direction, intersection.Normal, light_distance * EPSILON);
...
RGB WhittedShader::IndirectIllumination(const Ray& ray, const Scene& scene, const Intersection& intersection, const Material& material, int depth) const
{
  ...
  return fresnel * DoExecute(scattered_ray, scene, scattered_intersection, depth + 1);
}
} // namespace VI
